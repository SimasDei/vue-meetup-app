  ...
  return Meetup.aggregate([
    {
      $facet: {
        ...
        meetupsCount: [{ $match: { meetupCreator: userId } }, { $count: 'count' }],
      },
    },
  ])
    .exec()
    .then(results => {
      return new Promise(resolve => {
        Category.populate(results[0].meetups, { path: 'category' }).then(pMeetups => {
...
function fetchThreadsByUserQuery(userId) {
  return Thread.aggregate([
    {
      $facet: {
        threads: [{ $match: { user: userId } }, { $limit: 5 }, { $sort: { createdAt: -1 } }],
...
function fetchPostByUserQuery(userId) {
  return Post.aggregate([
    {
      $facet: {
        posts: [{ $match: { user: userId } }, { $limit: 5 }, { $sort: { createdAt: -1 } }],
